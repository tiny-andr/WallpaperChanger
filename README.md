...
- **Light / dark theme** — a small toggle button in the top-right corner of the main window switches themes on the spot and remembers the choice; the whole app follows, including the source manager, the manual picker (thumbnail grid included), the help window, the tray context menu and even the window title bar
...
- **Manual wallpaper picker** — curate exactly which wallpapers rotate: open the picker (button on the main window or tray menu), tick the tiles you like in the 16:9 grid (live file-name filter plus select-all / select-none / invert acting on the filtered set, and — when several sources feed the picker — a source filter so you can pick per source; reopening the window starts on the checked view) and hit save. Checking any wallpaper turns manual mode on; unchecking all of them turns it off. While active, auto rotation and "next" only draw from the checked set; random / order modes keep working on that smaller pool. Selections persist across restarts; newly added images default to unchecked, and "previous" remains a pure history walk.
