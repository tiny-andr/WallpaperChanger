...
- **多文件夹壁纸源，可逐个禁用** — 可添加多个文件夹，所有**已启用**来源的图片合并进同一个轮换池。主窗口的「壁纸源管理…」窗口会列出每个源及其壁纸数量（递归统计、含子文件夹），支持添加 / 删除 / 单独禁用：被禁用的源只是暂时不参与轮换，图片和勾选记录都会保留，重新勾选即恢复
- **6 种填充样式** — 填充 / 适应 / 拉伸 / 平铺 / 居中 / 跨屏（支持多显示器同图）
- **8 档切换间隔** — 1 / 5 / 10 / 30 分钟，1 / 6 / 12 / 24 小时
- **随机 / 顺序**两种轮换方式
- **多显示器** — 所有屏幕同步应用同一张壁纸
- **亮色 / 暗色主题** — 主窗口右上角的小按钮一键切换并立即记住；壁纸源管理、手动选择（含缩略图网格）、帮助窗口、托盘右键菜单乃至窗口标题栏都会跟随
- **全局快捷键**（主键盘**和小键盘**都支持）：
  - `Ctrl+9` — 下一张壁纸
  - `Ctrl+8` — 上一张壁纸（按本次会话的应用历史逐级回退；回退后再按 `Ctrl+9` 会先回到刚离开的那张）
- **手动壁纸选择** — 精确圈定参与轮换的壁纸：点主窗口的按钮（或托盘菜单）打开勾选窗口，在 16:9 缩略图网格里勾选喜欢的（支持文件名实时筛选，全选 / 全不选 / 反选作用于筛选结果；有多个壁纸源时顶部还有壁纸源筛选器，可按源分别挑选，再次打开默认停在"已选中的壁纸"视图），点保存即可。勾选了壁纸就开启手动模式，全部取消勾选即关闭；启用后，定时轮换和"下一张"只从勾选集合里挑，随机 / 顺序开关照常在这个小池子里工作；勾选状态重启后保持，之后新增的图片默认未勾选，"上一张"仍是纯历史回退、不受限制。
- **托盘常驻** — 右键托盘图标可 暂停 / 下一张 / 上一张 / 手动壁纸选择 / 退出；关闭窗口只是最小化到托盘
- **开机自启**（可选）— 开机时直接收进托盘、不弹出窗口；之后再次启动程序只是把已有窗口唤到前台
- **支持格式** — jpg / png / jfif / bmp / webp / gif / tiff；自动跳过 `Thumbs.db` 和损坏图片
- 中 / 英 / 日 三语界面，改动后需点击**保存设置**按钮才会写入配置文件（语言与主题属于即时生效，点选即存）
...
2. 打开设置窗口，点 **壁纸源管理** 按钮管理壁纸源文件夹：在管理窗口里用 **添加...** 加入文件夹、选中后 **删除选中** 移除，或勾选 / 取消勾选某一行来**启用 / 禁用**单个源（列表只读，不可手打）。每行会显示该源包含的壁纸数量。
...
更多细节（壁纸源管理 / 保存与后台 / 快捷键 / 手动壁纸选择 / 支持格式 / 新增壁纸生效逻辑）可在程序内点右下角**帮助**按钮查看。
