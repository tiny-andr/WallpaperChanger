...
- **手动壁纸选择** — 精确圈定参与轮换的壁纸：点壁纸源列表下方的按钮（或托盘菜单）打开勾选窗口，在 16:9 缩略图网格里勾选喜欢的（支持文件名实时筛选，全选 / 全不选 / 反选作用于筛选结果），点保存即可。勾选了壁纸就开启手动模式，全部取消勾选即关闭；启用后，定时轮换和"下一张"只从勾选集合里挑，随机 / 顺序开关照常在这个小池子里工作；勾选状态重启后保持，之后新增的图片默认未勾选，"上一张"仍是纯历史回退、不受限制。
